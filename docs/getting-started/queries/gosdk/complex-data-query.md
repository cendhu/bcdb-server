...
title: Query Data Using Fields in JSON value
---

Once a database session is created, we can call `session.JSONQuery()` to get the implementation of `Execute()` method.
To query a data using fields in JSON value, we need to use `Execute()` method.
